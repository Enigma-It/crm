...
@section('title', 'Diagnostic Patient Report Progress')
@section('content')
<section class="content-header">
  <h1><i class="fa fa-medkit"></i> Diagnostic Patient Report Progress</h1>
  <ol class="breadcrumb">
...
<section class="content">
  ...
  $btn_text = "Update Status"; $desplay_css= "none"; $date = date('m/d/Y'); 
  
  $btn_name = "save_print_btn";
?>

  @if( !empty($singlePurchaseData) )
        {{Form::open(array('route'=>['sample-status-update.update',$singlePurchaseData->id],'method'=>'PUT','files'=>true))}}
        <?php 
        ...
        ?>
  @else
        {{Form::open(array('route'=>['sample-status-update.store'],'method'=>'POST','files'=>true))}}
   @endif
               ...
               <table class="table table-responsive table-stripped table-bordered">
                  ...
                  <tfoot>
                    ...
                    <tr class="dynamicFooter">
                        ...
                        <td>
                            <input type="text" name="discount" id="discount" class="form-control decimal" readonly="">
                        </td>
...
<script type="text/javascript">
   ...
   function invoiceSearchByPatientID(value){
    ...
    if(value !=''){
        $.ajax({
            ...
            method: "POST",
            url: "{{URL::to('test-status-check-invoice')}}",
            data: {
              'invoice': value
            },
            dataType: "json",
            success: function(data){
              ...
              {   
                  ...
                  if(data.patientInfo.patient_gender == 2){
                    $('.p_sex').text('Female');
                  }
                  ...
                  $.each(data.patientList, function (index, value) {
                        ...
                        console.log(value.collection_status);
                        ...
                        html += "<td><input type='hidden' name='patient_test_id["+value.id+"]' /><select name='patient_test_id["+value.id+"][collection_status]' class='form-control'>";
                        if(value.collection_status ==1){
  ...
  function getTotal(){
    var receive_amount = due_amount = total_payable = 0;
